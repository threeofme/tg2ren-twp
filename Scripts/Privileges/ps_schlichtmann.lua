...
function InitOffice()
	SetOfficePrivileges( "Office", ps_schlichtmann_GetPrivilegeList() )
end
...
function TakeOffice(Messages)
	if (Messages == 1) then
		...
		feedback_MessageOffice("",
			ps_schlichtmann_GetPrivilegeList,
	...
	chr_SetOfficeImpactList( "Office", ps_schlichtmann_GetPrivilegeList() )
end
