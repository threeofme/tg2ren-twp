function Weight()
	...
	if not SimGetWorkingPlace("MYRM", "WorkBuilding") then
		return 0
	end
	...
	if MeasureNames[2] then
		...
		for i=0,Count-1 do
			State = DynastyGetDiplomacyState("MYRM", "Build"..i)
...
function Execute()
	MeasureRun("MYRM", "RivalBuild", "OrderASabotage_"..GetData("mysa_Measure"))
end
