...
function TransferItem(ItemId)
	...
	local Amount = 0 
	if HasProperty("Destination", "Amount") then
		Amount = GetProperty("Destination", "Amount")
		RemoveProperty("Destination", "Amount")
	end
	
	if Amount == 0 then
		Amount = GetItemCount("", ItemId, INVENTORY_STD)
		if Amount > 20 then
			Amount = 20
		end
	end
	
	if RemoveItems("", ItemId, Amount) > 0 then
		AddItems("Destination", ItemId, Amount)
	end
end
