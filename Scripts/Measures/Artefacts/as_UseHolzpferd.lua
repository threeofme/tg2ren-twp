function Run()
	...
	if RemoveItems("",ItemName,1)>0 then	
		...
		if GetImpactValue("Destination","ToyPferd")>0 then
			PlayAnimationNoWait("Destination","shake_head")
			MsgSay("Destination","@L_MEASURE_TOYPFERD_REJECT_FIRST")
			Sleep(1.5)
			...
			AddImpact("Destination", "ToyPferd", 1, 24)
			...
			PlayAnimation("Destination","child_jump")
