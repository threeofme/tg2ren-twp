...
function Run()
	...
	if (GetState("", STATE_CUTSCENE)) then
		as_usegoldchain_Cutscene()
	else
		as_usegoldchain_Normal()
	end
end
...
function Normal()
	...
	if RemoveItems("","GoldChain",1)>0 then	
		SetRepeatTimer("", GetMeasureRepeatName2("UseGoldChain"), TimeOut)
