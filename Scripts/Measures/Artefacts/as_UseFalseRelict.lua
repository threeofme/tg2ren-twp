-------------------------------------------------------------------------------
----
----	OVERVIEW "as_UseFalseRelict.lua"
...
function Run()
	...
	if choice == 0 then
		...
		if GetImpactValue("","Statue")>0 then
			...
			if RemoveItems("",Item,1)>0 then
				...
				AddImpact("","Attractivity",0.50,Duration)
				AddImpact("","FalseRelict",1,Duration)
			else
				StopMeasure()
			end
		end
