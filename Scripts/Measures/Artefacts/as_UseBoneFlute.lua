function Run()
	...
	if RemoveItems("",ItemName,1)>0 then	
		...
		if GetImpactValue("Destination","ToyFlute")>0 then
			PlayAnimationNoWait("Destination","shake_head")
			MsgSay("Destination","@L_MEASURE_TOYFLUTE_REJECT_FIRST")
			Sleep(1.5)
			...
			AddImpact("Destination", "ToyFlute", 1, 24)
			...
			Sleep(DestinationAnimationLength)
			local AnimTime = PlayAnimationNoWait("Destination","play_instrument_01_in")
			local EndTime = GetGametime() + 0.2
			Sleep(1)
			CarryObject("Destination","Handheld_Device/ANIM_Flute.nif",false)
			Sleep(AnimTime-1)
			while GetGametime() < EndTime do
				PlayAnimation("Destination","play_instrument_01_loop")
			end
			AnimTime = PlayAnimationNoWait("Destination","play_instrument_01_out")
			Sleep(1.5)
			CarryObject("Destination","",false)
		end
	end
