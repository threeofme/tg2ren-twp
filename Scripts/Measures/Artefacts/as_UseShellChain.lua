...
function Run()
	...
	if (GetState("", STATE_CUTSCENE)) then
		as_useshellchain_Cutscene()
	else
		as_useshellchain_Normal()
	end
end
...
function Normal()
	...
	if RemoveItems("","Shellchain",1)>0 then	
		SetRepeatTimer("", GetMeasureRepeatName2("UseShellChain"), TimeOut)
