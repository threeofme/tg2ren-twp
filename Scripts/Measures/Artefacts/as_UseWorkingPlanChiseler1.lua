-------------------------------------------------------------------------------
----
----	OVERVIEW "as_UseWorkingPlanChiseler1"
----
...
function Run()
	...
	if not GetInsideBuilding("", "Building") then
		FindNearestBuilding("", -1, 15, -1, false, "Building1")
		FindNearestBuilding("", -1, 36, -1, false, "Building2")
	...
	local Plan = "WorkingPlanChiseler1"
	local PlanCount = 1
	local PlanLabel = ItemGetLabel(Plan, true)
	local Object = "PoisonDagger"
	local ObjectCount = 1
	local ObjectLabel = ItemGetLabel(Object, true)

	local ItemName1 = "Silver"
	local ItemCount1 = 1
	local ItemLabel1
	local ItemName2 = "Tool"
	local ItemCount2 = 1
	local ItemLabel2
	local ItemName3 = "Dagger"
	local ItemCount3 = 1
	local ItemLabel3
	...
	local ProduceTime = 240 - (GetSkillValue("",SHADOW_ARTS) * 18)
