...
function Run()
	...
	if (GetState("", STATE_CUTSCENE)) then
		as_userubinstaff_Cutscene()
	else
		as_userubinstaff_Normal()
	end
end
...
function Normal()
	...
	if RemoveItems("","RubinStaff",1)>0 then	
		SetRepeatTimer("", GetMeasureRepeatName2("UseRubinStaff"), TimeOut)
