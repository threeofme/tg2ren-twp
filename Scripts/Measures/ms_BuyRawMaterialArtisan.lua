...
-- Mod End
	...
	local items = {"Iron", "Silver", "Gold", "Gemstone", "Grindingbrick", "Oakwood", "Pinewood", "Charcoal", "Pech", "Wool", "Leather", "Dye"}
	...
	for x = 1, itemcount-1 do
		...
		if items[x]=="Gemstone" then
		money[x] = math.floor(ItemGetBasePrice(items[x]) * (moneymod/2))*Rabatt
		else
	...
	if IsStateDriven() then
		...
		Result = InitData("@P"..btn,
				...
				ItemLabel[21],ItemLabel[22],
				ItemLabel[23],ItemLabel[24])
	end
	...
	elseif Result == "A9" then
		ItemIndex = 9
	elseif Result == "A10" then
		ItemIndex = 10
	elseif Result == "A11" then
		ItemIndex =11
	else
		ItemIndex = 12
	end
