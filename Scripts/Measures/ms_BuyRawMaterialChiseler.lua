...
-- Mod End
	...
	local npc = GetProperty("Guildhouse", "ChiselerElder")
	if npc==nil then
	...
	-- XXX Replace MediPack by Mead 
	local items = { 
			"Weapons", "Armors", "Mead", "CamouflageCloak"
			}
	...
	for x = 1, itemcount-1 do
		...
		if items[x]=="Weapons" then
			money[x] = math.floor(ItemGetBasePrice("Mace") * moneymod)*Rabatt
		elseif items[x]=="Armors" then
			money[x] = math.floor(ItemGetBasePrice("FullHelmet") * moneymod)*Rabatt
		else
	...
	--get the locators
	if not GetLocatorByName("Guildhouse","ChiselerOwner","OwnerPos") then
		StopMeasure()
	end
	if not GetLocatorByName("Guildhouse","ChiselerElder","ClerkPos") then
		StopMeasure()
	end
