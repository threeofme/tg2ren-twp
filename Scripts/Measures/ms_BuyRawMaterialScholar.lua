...
-- Mod End
	...
	local npc = GetProperty("Guildhouse", "ScholarElder")
	if npc==nil then
	...
	local items = { 
			"Pinewood", "Ink", "WheatFlour", "Wool", "Pech", "Fat", "Leather", "Fruit", "Honey", "Fungi"
			}
...
-- Fajeth Mod
	...
	--get the locators
	if not GetLocatorByName("Guildhouse","ScholarOwner","OwnerPos") then
		StopMeasure()
	end
	if not GetLocatorByName("Guildhouse","ScholarElder","ClerkPos") then
		StopMeasure()
	end
